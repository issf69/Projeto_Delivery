...
def ver_carrinho(request):
    ...
    carrinho = request.session.get('carrinho', [])  # Obtém o carrinho ou retorna uma lista vazia se não estiver definido
    
    for item in carrinho:
        prod = Produto.objects.filter(id=item['id_produto']).first()  # Use .first() para obter o primeiro item, caso exista
        if prod:
